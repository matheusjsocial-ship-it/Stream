...
export default function App(){
  ...
  const [mesesAssin,setMesesAssin]=useState(1);
  const [showAd,setShowAd]=useState(false);
  const [showUpsell,setShowUpsell]=useState(false);
  const [adCountdown,setAdCountdown]=useState(5);
  const [adSkippable,setAdSkippable]=useState(false); // meses de assinatura (virá do banco)
  ...
  useEffect(()=>{
    if(!showAd) return;
    const t=setInterval(()=>setAdCountdown(n=>{
      if(n<=1){setAdSkippable(true);clearInterval(t);return 0;}
      return n-1;
    }),1000);
    return()=>clearInterval(t);
  },[showAd]);
...
If not a recommendation request, answer normally. Use emojis. Respond in language code: ${uiLang}.`;
  ...
  function handleAuthSuccess(userData){
    ...
    setShowAuth(false);
    // Show upsell to basic plan users on login
    if(plan==="basic") setTimeout(()=>setShowUpsell(true), 800);
  }
  function addFriend(){
    ...
    <div style={{maxWidth:1160,margin:"0 auto",padding:"13px 14px",display:"flex",gap:11,flexWrap:"wrap"}}>
      <div style={{flex:1,minWidth:0}}>
        <div style={{background:`linear-gradient(155deg,${sel.g[0]}22,#06060e 70%)`,borderRadius:13,aspectRatio:"16/9",position:"relative",overflow:"hidden",border:"1px solid #1a1a36",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:7}}>
          ...
          {/* ── PRE-ROLL AD ── */}
          {showAd&&(
            <div style={{position:"absolute",inset:0,zIndex:50,display:"flex",flexDirection:"column",background:"#000"}} className="fade">
              {/* Ad creative */}
              <div style={{flex:1,background:"linear-gradient(135deg,#06060e 0%,#1a1a36 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,padding:20,position:"relative"}}>
                <div style={{position:"absolute",top:10,left:12,background:"rgba(255,255,255,.1)",borderRadius:4,padding:"2px 8px",fontSize:9,color:"rgba(255,255,255,.5)"}}>📢 Anúncio</div>
                <div style={{width:70,height:70,background:"linear-gradient(135deg,#f97316,#ef4444)",borderRadius:18,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,boxShadow:"0 8px 24px rgba(249,115,22,.4)"}}>🎬</div>
                <div style={{fontFamily:"Bebas Neue",fontSize:26,letterSpacing:2,color:"white",textAlign:"center"}}>LINGUA<span style={{color:"#f97316"}}>STREAM</span></div>
                <div style={{fontSize:13,color:"#94a3b8",textAlign:"center",lineHeight:1.7,maxWidth:280}}>
                  Aprenda idiomas assistindo o que ama!<br/>
                  Remova os anúncios por apenas
                </div>
                <div style={{background:"rgba(34,197,94,.1)",border:"1px solid rgba(34,197,94,.3)",borderRadius:12,padding:"10px 24px",textAlign:"center"}}>
                  <div style={{fontFamily:"Bebas Neue",fontSize:32,color:"#22c55e",letterSpacing:1}}>R$0,99<span style={{fontSize:14,color:"#64748b"}}>/mês</span></div>
                  <div style={{fontSize:10,color:"#64748b",marginTop:2}}>Básico — sem anúncios para sempre</div>
                </div>
                <button onClick={()=>{setShowAd(false);setPage("plans");}} style={{background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"white",padding:"10px 24px",borderRadius:20,fontWeight:800,cursor:"pointer",fontSize:13,boxShadow:"0 4px 14px rgba(34,197,94,.3)"}}>
                  Remover anúncios →
                </button>
              </div>
              {/* Bottom bar */}
              <div style={{background:"rgba(0,0,0,.9)",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderTop:"1px solid #1a1a36"}}>
                <div style={{fontSize:10,color:"#475569"}}>
                  {sel.type==="movie"?"🎬 Filme":"📺 Episódio"} — {sel.title}
                </div>
                {adSkippable
                  ?<button onClick={()=>{setShowAd(false);setPlaying(true);}} style={{background:"rgba(249,115,22,.15)",border:"1px solid rgba(249,115,22,.4)",color:"#f97316",padding:"5px 14px",borderRadius:16,fontWeight:800,cursor:"pointer",fontSize:11}}>
                    Pular anúncio →
                  </button>
                  :<div style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:16,padding:"5px 14px",fontSize:11,color:"#64748b"}}>
          ...
          {!playing&&(
            <div onClick={()=>{
                if(plan==="free"){
                  // Pre-roll on ALL content (movies too)
                  // Series/anime: every episode start
                  // Movies: once per movie
                  setShowAd(true);
                  setAdCountdown(5);
              ...
              }} style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexDirection:"column",gap:8}}>
    ...
    <div style={{maxWidth:800,margin:"0 auto",padding:"22px 14px"}} className="langFade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:11}}>
        {[
          ...
          {key:"basic",   lbl:t.basic||"Básico",sym:cData?.sym,price:cData?.p0||"0,99",col:"#22c55e",emoji:"📺",  inc:t.inc.basic||t.inc.free, no:t.no.basic||[], suf:t.month},
          {key:"premium", lbl:t.premium,        sym:cData?.sym,price:cData?.p1,col:"#f97316",emoji:"💜",  inc:t.inc.prem,  no:t.no.prem,  suf:t.month},
          {key:"agent",   lbl:t.agent,          sym:cData?.sym,price:cData?.p2,col:"#a855f7",emoji:"🤖",  inc:t.inc.agent, no:t.no.agent, suf:t.month,pop:true,upgrade:plan==="basic"},
        ].map(p=>(
          <div key={p.key} style={{background:"#0d0d1a",border:`2px solid ${plan===p.key?p.col:"#1a1a36"}`,borderRadius:13,padding:15,position:"relative",transition:"border .18s"}}>
            ...
            {p.upgrade&&<div style={{position:"absolute",top:-9,right:12,background:"linear-gradient(135deg,#f97316,#ef4444)",color:"white",padding:"2px 10px",borderRadius:14,fontSize:9,fontWeight:800,animation:"pulse 2s ease infinite"}}>⬆️ UPGRADE</div>}
            <div style={{textAlign:"center",marginBottom:11}}>
  ...
  /* ── UPSELL MODAL — aparece para usuários do plano Básico ── */
  const UpsellModal=()=>!showUpsell?null:(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:2500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} className="fade">
      <div style={{width:"min(420px,100%)",background:"#0d0d1a",borderRadius:20,overflow:"hidden",border:"1px solid rgba(168,85,247,.3)",boxShadow:"0 20px 60px rgba(168,85,247,.2)"}}>
        {/* Header */}
        <div style={{background:"linear-gradient(135deg,#a855f7,#6366f1)",padding:"20px",textAlign:"center",position:"relative"}}>
          <button onClick={()=>setShowUpsell(false)} style={{position:"absolute",top:10,right:12,background:"rgba(255,255,255,.15)",border:"none",color:"white",fontSize:16,cursor:"pointer",borderRadius:6,padding:"2px 8px"}}>✕</button>
          <div style={{fontSize:36,marginBottom:6}}>🤖</div>
          <div style={{fontFamily:"Bebas Neue",fontSize:22,letterSpacing:1,color:"white"}}>CONHEÇA O PLANO AGENTE</div>
          <div style={{fontSize:11,color:"rgba(255,255,255,.8)",marginTop:2}}>Desbloqueie o ORION e muito mais</div>
        </div>
        {/* Content */}
        <div style={{padding:18}}>
          {/* Comparison */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            {/* Basic */}
            <div style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:11,padding:12,opacity:.7}}>
              <div style={{fontFamily:"Bebas Neue",fontSize:14,color:"#64748b",marginBottom:6}}>📺 Básico (atual)</div>
              {["Legenda dupla PT+EN+JP","720p","Sem anúncios"].map(f=>(
                <div key={f} style={{fontSize:10,color:"#64748b",marginBottom:3,display:"flex",gap:4}}>
                  <span>✓</span>{f}
                </div>
              ))}
              <div style={{fontFamily:"Bebas Neue",fontSize:18,color:"#64748b",marginTop:8}}>R$0,99</div>
            </div>
            {/* Agent */}
            <div style={{background:"rgba(168,85,247,.08)",border:"2px solid #a855f7",borderRadius:11,padding:12,position:"relative"}}>
              <div style={{position:"absolute",top:-8,left:"50%",transform:"translateX(-50%)",background:"linear-gradient(135deg,#a855f7,#6366f1)",color:"white",padding:"1px 10px",borderRadius:10,fontSize:8,fontWeight:800,whiteSpace:"nowrap"}}>⭐ UPGRADE</div>
              <div style={{fontFamily:"Bebas Neue",fontSize:14,color:"#a855f7",marginBottom:6}}>🤖 Agente</div>
              {["ORION ilimitado","Áudio 12 idiomas","4K + Download","Watch Party","Quiz ao vivo"].map(f=>(
                <div key={f} style={{fontSize:10,color:"#e2e8f0",marginBottom:3,display:"flex",gap:4}}>
                  <span style={{color:"#a855f7"}}>✓</span>{f}
                </div>
              ))}
              <div style={{fontFamily:"Bebas Neue",fontSize:18,color:"#a855f7",marginTop:8}}>R$9,99</div>
            </div>
          </div>

          {/* ORION highlight */}
          <div style={{background:"rgba(168,85,247,.07)",border:"1px solid rgba(168,85,247,.2)",borderRadius:11,padding:12,marginBottom:14,display:"flex",gap:10,alignItems:"center"}}>
            <div style={{width:36,height:36,background:"linear-gradient(135deg,#a855f7,#6366f1)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🤖</div>
            <div>
              <div style={{fontWeight:800,fontSize:12,color:"#a855f7"}}>ORION — Professor de Idiomas IA</div>
              <div style={{fontSize:10,color:"#64748b",marginTop:2,lineHeight:1.5}}>Tire dúvidas de vocabulário, gramática e cultura em tempo real enquanto assiste!</div>
            </div>
          </div>

          {/* CTA */}
          <button onClick={()=>{setShowUpsell(false);setCheckoutPlano("agent");setShowCheckout(true);}} style={{width:"100%",padding:"13px",background:"linear-gradient(135deg,#a855f7,#6366f1)",border:"none",borderRadius:11,color:"white",fontWeight:800,cursor:"pointer",fontSize:14,boxShadow:"0 6px 20px rgba(168,85,247,.35)",marginBottom:8}}>
            🚀 Fazer upgrade por R$9,99/mês
          </button>
          <button onClick={()=>setShowUpsell(false)} style={{width:"100%",padding:"8px",background:"transparent",border:"none",color:"#475569",cursor:"pointer",fontSize:11}}>
            Continuar com o Básico
          </button>
          <div style={{textAlign:"center",fontSize:9,color:"#1e1e3a",marginTop:6}}>Cancele quando quiser · Sem fidelidade</div>
        </div>
      </div>
    </div>
  );
