...
const I18N = {
  PT: {
    ...
    startFree:"🚀 Começar Grátis", recommend:"🤖 Me Recomendar",
    ...
    free:"Gratuito", premium:"Premium", agent:"Agente", mostPop:"⭐ MAIS POPULAR",
    ...
    inc:{
      free:["Catálogo básico","1 legenda","Qualidade 480p","Com anúncios"],
      prem:["Tudo grátis","Legenda dupla 12 idiomas","Sem anúncios","HD 1080p","Trailer integrado","Watch Party 2 pessoas","Missões com desconto"],
      agent:["Tudo do Premium","Áudio 12 idiomas","ORION — Guia de Idiomas","Quiz ao vivo +XP","Watch Party ilimitada","Recomendações IA","Clipes compartilháveis","4K + Download offline"],
    },
    no:{
      free:["Legenda dupla","Áudio multi-idioma","Sem anúncios","ORION","Quiz","Recomendações IA"],
      prem:["Áudio multi-idioma","ORION","Quiz ao vivo","Download offline"],
      agent:[],
    },
  ...
  EN: {
    ...
    startFree:"🚀 Start Free", recommend:"🤖 Recommend Me",
    ...
    free:"Free", premium:"Premium", agent:"Agent", mostPop:"⭐ MOST POPULAR",
  ...
  JP: {
    ...
    startFree:"🚀 無料で始める", recommend:"🤖 おすすめを見る",
    ...
    free:"無料", premium:"プレミアム", agent:"エージェント", mostPop:"⭐ 人気No.1",
  ...
  KR: {
    ...
    startFree:"🚀 무료 시작", recommend:"🤖 추천 받기",
    ...
    free:"무료", premium:"프리미엄", agent:"에이전트", mostPop:"⭐ 가장 인기",
  ...
  ES: {
    ...
    startFree:"🚀 Empezar Gratis", recommend:"🤖 Recomiéndame",
    ...
    free:"Gratis", premium:"Premium", agent:"Agente", mostPop:"⭐ MÁS POPULAR",
  ...
  FR: {
    ...
    startFree:"🚀 Commencer Gratuitement", recommend:"🤖 Me Recommander",
    ...
    free:"Gratuit", premium:"Premium", agent:"Agent", mostPop:"⭐ LE PLUS POPULAIRE",
  ...
  DE: {
    ...
    startFree:"🚀 Kostenlos Starten", recommend:"🤖 Empfehlung",
    ...
    free:"Kostenlos", premium:"Premium", agent:"Agent", mostPop:"⭐ BELIEBTESTE",
  ...
  HI: {
    ...
    startFree:"🚀 मुफ़्त शुरू करें", recommend:"🤖 सुझाव दें",
    ...
    free:"मुफ़्त", premium:"प्रीमियम", agent:"एजेंट", mostPop:"⭐ सबसे लोकप्रिय",
...
const CURRENCIES = [
  {code:"BRL",sym:"R$",p1:"0,99",p2:"1,99",country:"🇧🇷 Brasil"},
  {code:"USD",sym:"$", p1:"0.99",p2:"1.99",country:"🇺🇸 USA"},
  {code:"EUR",sym:"€", p1:"0.99",p2:"1.99",country:"🇪🇺 Europe"},
  {code:"JPY",sym:"¥", p1:"149", p2:"299", country:"🇯🇵 日本"},
  {code:"KRW",sym:"₩", p1:"1.290",p2:"2.590",country:"🇰🇷 한국"},
  {code:"MXN",sym:"$", p1:"18",  p2:"35",  country:"🇲🇽 México"},
  {code:"INR",sym:"₹", p1:"29",  p2:"59",  country:"🇮🇳 भारत"},
];
...
async function askClaude(messages,system){
  try{
    ...
    const d=await res.json();return d.content?.[0]?.text||"Erro.";
...
export default function App(){
  ...
  const [form,setForm]=useState({nome:"",email:"",senha:"",cel:"",wp:false,lgpd:false});
  // friends system
  ...
  async function sendChat(){
    ...
    setMsgs(p=>[...p,{role:"assistant",content:text}]);setLoading(false);
  }
  ...
  async function sendDiscover(){
    ...
    const sys=`You are ORION, LinguaStream's trusted guide and curator. Catalog: ${TITLES}.
...
If not a recommendation request, answer normally. Use emojis. Respond in language code: ${uiLang}.`;
  ...
  function handleAuth(e){
    ...
    setUser({nome:form.nome||"Zinho",email:form.email,wp:form.wp,cel:form.cel,lsId});
    setShowAuth(false);setForm({nome:"",email:"",senha:"",cel:"",wp:false,lgpd:false});
  }
  function addFriend(){
  ...
  const AuthModal=()=>showAuth?(
    <div onClick={()=>setShowAuth(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:1800,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} className="fade">
      <div onClick={e=>e.stopPropagation()} style={{width:"min(400px,100%)",background:"#0d0d1a",borderRadius:18,overflow:"hidden",border:"1px solid #1a1a36"}}>
        <div style={{background:"linear-gradient(135deg,#f97316,#a855f7)",padding:"18px 20px",textAlign:"center"}}>
          <div style={{fontFamily:"Bebas Neue",fontSize:24,letterSpacing:1}}>LINGUA<span style={{color:"white"}}>STREAM</span></div>
          <div style={{fontSize:11,opacity:.85,marginTop:1}}>{authMode==="login"?t.authWelcome:t.authCreate}</div>
        </div>
        <div style={{padding:20}}>
          <div style={{display:"flex",background:"#111128",borderRadius:9,padding:3,marginBottom:15}}>
            {[["login",t.signin],["register",t.signup]].map(([m,l])=>(
          ...
          <form onSubmit={handleAuth} style={{display:"flex",flexDirection:"column",gap:9}}>
            {authMode==="register"&&<input required value={form.nome} onChange={e=>setForm(p=>({...p,nome:e.target.value}))} placeholder={t.name} style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:8,padding:"9px 11px",color:"#e2e8f0",fontSize:12,outline:"none"}}/>}
            <input required type="email" value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))} placeholder={t.email} style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:8,padding:"9px 11px",color:"#e2e8f0",fontSize:12,outline:"none"}}/>
            <input required type="password" value={form.senha} onChange={e=>setForm(p=>({...p,senha:e.target.value}))} placeholder={t.pass} style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:8,padding:"9px 11px",color:"#e2e8f0",fontSize:12,outline:"none"}}/>
            {authMode==="register"&&(
              <>
                <input type="tel" value={form.cel} onChange={e=>setForm(p=>({...p,cel:e.target.value}))} placeholder={t.phone} style={{background:"#111128",border:"1px solid #1a1a36",borderRadius:8,padding:"9px 11px",color:"#e2e8f0",fontSize:12,outline:"none"}}/>
                <label style={{display:"flex",alignItems:"flex-start",gap:8,cursor:"pointer",background:"rgba(37,211,102,.06)",border:"1px solid rgba(37,211,102,.2)",borderRadius:8,padding:"8px 10px"}}>
                  <input type="checkbox" checked={form.wp} onChange={e=>setForm(p=>({...p,wp:e.target.checked}))} style={{marginTop:2,accentColor:"#25d366",flexShrink:0}}/>
                  <span style={{fontSize:10,lineHeight:1.5,color:"#94a3b8"}}>{t.wpConsent}</span>
                </label>
                <label style={{display:"flex",alignItems:"flex-start",gap:8,cursor:"pointer"}}>
                  <input required type="checkbox" checked={form.lgpd} onChange={e=>setForm(p=>({...p,lgpd:e.target.checked}))} style={{marginTop:2,accentColor:"#f97316",flexShrink:0}}/>
                  <span style={{fontSize:10,lineHeight:1.5,color:"#64748b"}}>{t.terms}</span>
                </label>
              </>
            )}
          ...
          {authMode==="register"&&<div style={{marginTop:9,background:"rgba(100,116,139,.06)",border:"1px solid #1a1a36",borderRadius:7,padding:"6px 9px",fontSize:9,color:"#64748b",lineHeight:1.5}}>{t.lgpdNote}</div>}
        </div>
      </div>
    </div>
  ):null;
      ...
      <div style={{width:272,flexShrink:0,display:"flex",flexDirection:"column",gap:9}}>
        ...
        <div style={{background:"#0d0d1a",borderRadius:10,border:"1px solid #1a1a36",display:"flex",flexDirection:"column",flex:1,minHeight:310}}>
          <div style={{padding:"8px 11px",borderBottom:"1px solid #1a1a36",display:"flex",alignItems:"center",gap:6}}>
            ...
            <div><div style={{fontWeight:800,fontSize:11}}>ORION</div><div style={{fontSize:8,color:"#22c55e",fontWeight:700}}>{t.online}</div></div>
  ...
  const Plans=()=>(
    <div style={{maxWidth:800,margin:"0 auto",padding:"22px 14px"}} className="langFade">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:11}}>
        {[
          {key:"free",   lbl:t.free,   sym:"",       price:"0",      col:"#64748b",emoji:t.planLabel[0],inc:t.inc.free,  no:t.no.free},
          {key:"premium",lbl:t.premium,sym:cData?.sym,price:cData?.p1,col:"#f97316",emoji:t.planLabel[1],inc:t.inc.prem,  no:t.no.prem,suf:t.month},
          {key:"agent",  lbl:t.agent,  sym:cData?.sym,price:cData?.p2,col:"#a855f7",emoji:t.planLabel[2],inc:t.inc.agent, no:t.no.agent,suf:t.month,pop:true},
        ].map(p=>(
          <div key={p.key} style={{background:"#0d0d1a",border:`2px solid ${plan===p.key?p.col:"#1a1a36"}`,borderRadius:13,padding:15,position:"relative",transition:"border .18s"}}>
            ...
            <div style={{textAlign:"center",marginBottom:11}}>
              ...
              <div style={{marginTop:3}}><span style={{fontFamily:"Bebas Neue",fontSize:30,color:"white"}}>{p.sym}{p.price}</span>{p.suf&&<span style={{color:"#64748b",fontSize:10}}>{p.suf}</span>}</div>
            </div>
            ...
            <button onClick={()=>{setPlan(p.key);if(!user){setAuthMode("register");setShowAuth(true);}}} style={{width:"100%",padding:"8px",background:plan===p.key?`linear-gradient(135deg,${p.col},${p.key==="agent"?"#6366f1":"#ef4444"})`:"transparent",border:`1px solid ${p.col}`,borderRadius:8,color:plan===p.key?"white":p.col,fontWeight:800,cursor:"pointer",fontSize:11}}>
              {plan===p.key?t.activePlan:`${t.subscribe} ${p.lbl}`}
            </button>
          </div>
        ))}
      </div>
