...
export default function App(){
  ...
  const [favorites,setFavorites]=useState(new Set([1,2,8,9])); // IDs favoritos
        ...
        {[
          ...
          <div key={p.key} style={{background:"#0d0d1a",border:`2px solid ${plan===p.key?p.col:"#1a1a36"}`,borderRadius:13,padding:15,position:"relative",transition:"border .18s"}}>
            ...
            <button onClick={()=>{
              ...
              {plan===p.key?t.activePlan:p.key==="free"?(user?"✓ Plano Atual":(t.signup||"Criar Conta")):`${t.subscribe} ${p.lbl}`}
            </button>
          </div>
        ))}
      </div>
