...
const CURRENCIES = [
  {code:"BRL",sym:"R$",p0:"1,99", p1:"6,90", p2:"12,90",country:"🇧🇷 Brasil"},
  {code:"USD",sym:"$", p0:"0.49", p1:"1.49", p2:"2.99", country:"🇺🇸 USA"},
  {code:"EUR",sym:"€", p0:"0.45", p1:"1.29", p2:"2.49", country:"🇪🇺 Europe"},
  {code:"JPY",sym:"¥", p0:"75",   p1:"220",  p2:"420",  country:"🇯🇵 日本"},
  {code:"KRW",sym:"₩", p0:"690",  p1:"1.990",p2:"3.990",country:"🇰🇷 한국"},
  {code:"MXN",sym:"$", p0:"10",   p1:"28",   p2:"54",   country:"🇲🇽 México"},
  {code:"INR",sym:"₹", p0:"19",   p1:"49",   p2:"99",   country:"🇮🇳 भारत"},
];
    ...
    <div style={{minHeight:"100vh",background:"#06060e",color:"#e2e8f0",fontFamily:"Nunito,sans-serif",overflowX:"hidden"}}>
      ...
      <div style={{position:"relative",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 20px",textAlign:"center",overflow:"hidden"}}>
        ...
        <div className="ls-fade" style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(34,197,94,.08)",border:"1px solid rgba(34,197,94,.25)",borderRadius:20,padding:"6px 16px",marginBottom:32}}>
          ...
          <span style={{width:1,height:12,background:"rgba(34,197,94,.3)"}}/>
...
If not a recommendation request, answer normally. Use emojis. Respond in language code: ${uiLang}.`;
  ...
  const Plans=()=>(
    <div style={{maxWidth:800,margin:"0 auto",padding:"22px 14px"}} className="langFade">
      ...
      {user&&(()=>{
        const {tempo,miss,total} = calcDescontoTotal(mesesAssin, missoesFeitas);
        return total>0?(
          <div style={{background:"linear-gradient(135deg,rgba(34,197,94,.08),rgba(249,115,22,.08))",border:"1px solid rgba(34,197,94,.25)",borderRadius:12,padding:13,marginBottom:16,textAlign:"center"}} className="fade">
            ...
            <div style={{display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap",marginBottom:6}}>
              {tempo.perc>0&&<div style={{fontSize:12,color:"#22c55e",fontWeight:800}}>⏱️ {tempo.perc}% fidelidade ({mesesAssin} {mesesAssin===1?"mês":"meses"})</div>}
              {miss.perc>0&&<div style={{fontSize:12,color:"#f97316",fontWeight:800}}>🎯 {miss.perc}% missões ({missoesFeitas} completas)</div>}
            </div>
            <div style={{fontFamily:"Bebas Neue",fontSize:24,color:"white",letterSpacing:1}}>
              DESCONTO TOTAL: <span style={{color:"#22c55e"}}>{total}% OFF</span>
            </div>
      ...
      <div style={{background:"#0d0d1a",border:"1px solid #1a1a36",borderRadius:12,padding:14,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(90px,1fr))",gap:6}}>
          {TABELA_DESCONTOS.map(t=>(
            <div key={t.range} style={{textAlign:"center",padding:"7px 5px",borderRadius:8,background:t.destaque?"rgba(249,115,22,.1)":mesesAssin>0&&calcDescontoPorTempo(mesesAssin).perc>=t.desc&&t.desc>0?"rgba(34,197,94,.1)":"rgba(255,255,255,.02)",border:`1px solid ${t.destaque?"rgba(249,115,22,.3)":mesesAssin>0&&calcDescontoPorTempo(mesesAssin).perc>=t.desc&&t.desc>0?"rgba(34,197,94,.3)":"#1a1a36"}`}}>
              <div style={{fontSize:9,color:"#64748b",marginBottom:2}}>{t.range}</div>
              <div style={{fontFamily:"Bebas Neue",fontSize:16,color:t.desc===0?"#475569":t.destaque?"#f97316":"#22c55e"}}>{t.desc===0?"—":`${t.desc}%`}</div>
            </div>
          ))}
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(215px,1fr))",gap:11}}>
        {[
          ...
          {key:"basic",   lbl:t.basic||"Básico", sym:cData?.sym,price:cData?.p0,col:"#22c55e",emoji:"📺",  inc:t.inc.basic,  no:t.no.basic,  suf:t.month},
              ...
              {(()=>{
                if(p.key==="free") return(
                  <div style={{marginTop:3}}>
                    <span style={{fontFamily:"Bebas Neue",fontSize:30,color:"white"}}>{p.sym}0</span>
                    <span style={{color:"#64748b",fontSize:10}}> grátis</span>
                  </div>
                );
                const {total} = calcDescontoTotal(mesesAssin, missoesFeitas);
                const valorNum = parseFloat(p.price.replace(",","."));
                const valorFinal = valorNum * (1 - total/100);
                return(
                  <div style={{marginTop:3}}>
                    {total>0&&<div style={{fontSize:10,color:"#64748b",textDecoration:"line-through"}}>{p.sym}{p.price}</div>}
                    <span style={{fontFamily:"Bebas Neue",fontSize:30,color:total>0?"#22c55e":"white"}}>
                      {p.sym}{valorFinal.toFixed(2).replace(".",",")}
                    </span>
                    {p.suf&&<span style={{color:"#64748b",fontSize:10}}>{p.suf}</span>}
                    {total>0&&<div style={{fontSize:9,color:"#22c55e",fontWeight:800}}>{total}% OFF aplicado 🎉</div>}
                  </div>
                );
              })()}
              {p.key!=="free"&&(
